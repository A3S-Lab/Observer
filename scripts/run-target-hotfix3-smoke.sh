...
RUN_DIR=$(mktemp -d /tmp/anysentry-observer-hotfix3.XXXXXX)
...
echo "AnySentry Observer UOS kernel 4.19.90 hotfix3 smoke test"
...
if ! grep -q '^kernel_version_code=0x0004135a$' "$SCRIPT_DIR/PROVENANCE"; then
  fail "package is not marked for the detected UOS 4.19.90 BPF ABI"
fi
...
/bin/sh -c 'echo anysentry-hotfix3-smoke >/dev/null'
...
if (( failure != 0 )); then
  fail "hotfix3 did not pass on the detected UOS kernel"
fi

echo "RESULT=PASS: UOS 4.19.90 legacy Observer loaded probes and emitted events"
echo "logs=$RUN_DIR"
