...
{
  echo "artifact=anysentry-observer-linux-4.19.90-arm64-hotfix3"
  echo "observer_commit=$observer_commit"
  echo "target=$OBSERVER_TARGET"
  echo "backend=perf-kprobe-legacy"
  echo "bpf_isa=v1"
  echo "kernel_version=4.19.90"
  echo "kernel_version_code=0x0004135a"
...
install -m 0755 \
  "$ROOT_DIR/scripts/run-target-hotfix3-smoke.sh" \
