# AnySentry Observer UOS Linux 4.19.90 ARM64 hotfix3 替换说明
...
```bash
export HOTFIX_DIR=/opt/shannon/anysentry/v0.1.0/anysentry-observer-linux-4.19.90-arm64-hotfix3
...
同时确认本包使用目标机探测得到的 BPF 内核版本编码：

```bash
grep -E '^kernel_version(_code)?=' PROVENANCE
```
...
kernel_version_code=0x0004135a
```
...
保存在脚本打印的 `logs=/tmp/anysentry-observer-hotfix3.*` 目录中。

## 3. 备份并原子替换 collector

```bash
install -d -m 0755 /opt/anysentry/observer/bin
backup=/opt/anysentry/observer/bin/a3s-observer-collector.before-linux41990-hotfix3.$(date +%Y%m%d%H%M%S)
...
```bash
backup=$(ls -1t /opt/anysentry/observer/bin/a3s-observer-collector.before-linux41990-hotfix3.* 2>/dev/null | head -n 1)
export backup
